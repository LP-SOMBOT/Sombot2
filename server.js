...
function initializeBotListeners() {
    ...
    async function startBot(uid) {
        ...
        sock.ev.on('connection.update', async (update) => {
            ...
            if (connection === 'open') {
                ...
                const welcomeMessage = `*Night Wa Bot </> is now online!* ✨\n\n` +
                    `Type .menu to see all available commands. If you need help, please join our support group.\n\n` +
        ...
        sock.ev.on('messages.upsert', async (m) => {
            // --- NEW: Added a try-catch block for stability ---
            try {
    ...
    db.collection('bots').onSnapshot(snapshot => {
        snapshot.docChanges().forEach(change => {
            ...
            if (change.type === 'removed' && activeBots.has(uid)) {
                activeBots.get(uid).logout(); activeBots.delete(uid);
            }
        });
    });
}
